...
export const OptionsPost: FC<OptionsPostProps> =
  ({ commentsNumber, title, subtitle, idPost, bookmarked, favorited, countFavorites, countBookmarks }) => {
    ...
    return <div className='bg-background/90 border-foreground/5 dark:border-foreground/10 sticky bottom-0 z-10 mx-auto flex w-full max-w-[650px] flex-wrap items-center gap-4 border-y p-2 px-4 backdrop-blur-md'>
      ...
      <button
        disabled={isLoading || !user?.email} onClick={toggleFavorite}
        className={`flex cursor-pointer items-center gap-1 ${!favorites && "opacity-40 hover:opacity-100"} active:scale-90`}>
        <Heart
      ...
      <button
        ...
        className='flex cursor-pointer items-center gap-1  opacity-40 transition-all hover:opacity-100 active:scale-90'>
        <Bookmark
